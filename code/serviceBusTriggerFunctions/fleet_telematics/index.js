 ...
 */
 ...
 /*
 * HERE Serverless Function  :  Fleet Telematics API.
 */
...
const CONTAINER_ID  = config.cosmosDB.containerId_fleetTelematics;
...
module.exports = async function(context, serviceBusMsg) {
  ...
  //input reuest object/serviceBusMsg should be in JSON format  
  ...
  */
  ...
  if (! reqProcessor.validateIncomingMsg(request, serviceBusMsg)){
    logger("[ERROR] :Incoming ServiceBusMessage Validation Failed");
    reqProcessor.createDBLog(logger,request);
    return; 
  }
  ...
  request._proxy  = reqProcessor.processServiceBusRequest(logger, request );

  //Invoke Proxy URL and fetch Response, GET/POST call is decided based on incoming method.
  request._result = await reqProcessor.getAPIResult(logger,request);
