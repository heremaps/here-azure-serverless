...
//Process incoming service bus message and log incoming parameters.
function processServiceBusRequest(log, req ) {
...
function createDBLog(log,req) {
  ...
  var logEntry  = cosmosdblogger.prepareServiceBusLog(req);
  log("[INFO] : Service Bus Msg Log for COSMOS DB prepared successfully. ");
...
//validate incoming servicebus message for its JSON wellformedness.
function validateIncomingMsg(req,serviceBusMsg) {
  try {
    //parse incoming message and merge with existing req object.
    req = Object.assign(req,JSON.parse(JSON.stringify(serviceBusMsg)));
    return true;
  }
  catch(err){
    req._error = "JSON parsing for incoming service bus msg failed,[" +  err  + "]";
    return false;
  }
}
...
module.exports = {
  processServiceBusRequest: processServiceBusRequest,
